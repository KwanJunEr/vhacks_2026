...
export default function DroneDetailPage() {
  ...
  if (isLoading)
    return (
      <div className="p-12 text-emerald-500 font-mono">
        ACCESSING ENCRYPTED DATA...
      </div>
    );
  if (!drone)
    return (
      <div className="p-12 text-red-500 font-mono">DRONE UNIT NOT FOUND</div>
    );
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-gradient-to-br from-[#0a1a10] to-[#061209] rounded-3xl border border-[#0d2e18] p-4 h-[400px] shadow-2xl relative overflow-hidden flex items-center justify-center">
            <div className="absolute top-6 left-6 z-10">
              <span className="text-[10px] font-black text-emerald-500/50 uppercase tracking-[0.3em]">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button className="py-3 px-4 bg-blue-50 text-blue-600 rounded-xl font-bold text-[10px] uppercase hover:bg-blue-100 transition-all">
                Manual Override
              </button>
              <button className="py-3 px-4 bg-emerald-50 text-emerald-600 rounded-xl font-bold text-[10px] uppercase hover:bg-emerald-100 transition-all">
                Reroute Mission
              </button>
              <button className="py-3 px-4 bg-amber-50 text-amber-600 rounded-xl font-bold text-[10px] uppercase hover:bg-amber-100 transition-all">
                Reboot Core
              </button>
              <button className="py-3 px-4 bg-red-50 text-red-600 rounded-xl font-bold text-[10px] uppercase hover:bg-red-100 transition-all">
                Emergency Land
              </button>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-slate-500 font-medium text-lg">
              {drone.brand} {drone.model} • {drone.profile}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={i}
                className="p-6 rounded-3xl bg-white border border-slate-100 shadow-sm space-y-3"
              >
                <div
