...
export default function EventDetailPage({
  ...
  // Fetch Drones - Mocked since Supabase is removed
  useEffect(() => {
    const fetchDrones = async () => {
      ...
      const droneStates: DroneState[] = mockData.map((d, i) => ({
        id: d.drone_name,
        label: d.drone_name,
  ...
  // Update battery - Mocked since Supabase is removed
  const updateBatteryInDB = async (droneName: string, battery: number) => {
    // console.log(`Mock DB Update: ${droneName} battery -> ${battery}%`);
  };
  ...
  const handleDeploy = async () => {
    ...
    addLog(
      "Reading environmental data: Ranau Seismograph Station 14...",
      "system",
    );
    await new Promise((r) => setTimeout(r, 1000));
    addLog(
      "Detected secondary tremors (Mag 3.4) in Ranau District.",
      "warning",
    );
    await new Promise((r) => setTimeout(r, 1000));
    ...
    addLog(
      `Discovery complete: [${droneList}] available via MCP Protocol.`,
      "success",
      true,
    );
    ...
    addLog(
      "Command Agent: Decomposing mission goals into tactical sectors.",
      "ai",
      true,
    );
    await new Promise((r) => setTimeout(r, 1500));
    ...
    await new Promise((r) => setTimeout(r, 1000));
    addLog(
      "Decision: Assigning Alpha-1 to A1, Alpha-2 to A7, Alpha-3 to G1, Alpha-4 to G7.",
      "ai",
      true,
    );
    ...
    addLog(
      "MCP: Broadasting move_to() commands. Swarm entering tactical grid.",
      "system",
    );
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (scannedCells.size === TOTAL_CELLS) {
        ...
        addLog(
          "MISSION COMPLETE: 100% tactical grid coverage achieved.",
          "success",
          true,
        );
        return;
      ...
      setDrones((prevDrones) => {
        ...
        const nextDrones = prevDrones.map((drone) => {
          ...
          if (drone.battery <= 0 || drone.status === "RECHARGING") {
            if (drone.status === "RECHARGING") {
              ...
              if (rechargedBattery === 100) {
                ...
                addLog(
                  `${drone.label}: Recharging complete. System diagnostics nominal. Will proceed to finish scanning to reach 100% grid coverage.`,
                  "success",
                  true,
                );
                return {
            ...
            if (drone.pos !== -1) {
              addLog(
                `ALERT: ${drone.label} battery depleted. Initiating emergency docking at Base Station.`,
                "warning",
              );
              changed = true;
          ...
          const unscannedMoves = validMoves.filter(
            (m) => !scannedCells.has(drone.pos + m),
          );
          ...
          if (!nextMove && unscannedMoves.length > 0) {
            nextMove =
              unscannedMoves[Math.floor(Math.random() * unscannedMoves.length)];
          }
          ...
          if (
            survivorSector !== null &&
            finalPos === survivorSector &&
            !survivorFound
          ) {
            setSurvivorFound(true);
            setShowSurvivorDialog(true);
            addLog(
              `AI DETECTION: ${drone.label} confirmed survivor in Sector ${finalPos}!`,
              "success",
              true,
            );
            return {
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
            ...
            <Badge
              variant="destructive"
              className="animate-pulse bg-red-600 px-3 py-1 font-bold shadow-lg shadow-red-500/20"
            >
              LIVE MISSION
            </Badge>
          </h1>
          <div className="flex items-center gap-4 text-sm text-slate-500 font-medium">
            <span className="flex items-center gap-1.5">
              <MapPin className="w-4 h-4 text-red-500" /> Ranau District,
              Malaysia
            </span>
            <span className="flex items-center gap-1.5">
